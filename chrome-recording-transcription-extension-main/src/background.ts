...
function setBadge(recording: boolean) {
  chrome.action
    .setBadgeText({ text: recording ? "REC" : "" })
    .catch?.(() => {});
}
...
async function hasOffscreenContext(): Promise<boolean> {
  try {
    const getContexts = (chrome.runtime as any).getContexts as
      | ((q: {
          contextTypes: ("OFFSCREEN_DOCUMENT" | string)[];
        }) => Promise<any[]>)
      | undefined;
    if (getContexts) {
...
chrome.runtime.onConnect.addListener((port) => {
  ...
  port.onMessage.addListener((msg: any) => {
    ...
    if (msg?.type === "RECORDING_STATE") {
      ...
      chrome.runtime
        .sendMessage({ type: "RECORDING_STATE", recording: lastKnownRecording })
        .catch(() => {});

      // If we just stopped recording, maybe we want to keep the transcript?
      // For now, we don't auto-clear it here to allow "Summarize" after stop.
    }
    ...
    if (msg?.type === "DEBUG_LOG") {
      console.log("[OFFSCREEN-DEBUG]", msg.message);
    }
  });
  ...
  (async () => {
    if (msg?.type === "START_RECORDING") {
      ...
      try {
        ...
        sendResponse({
          ok: false,
          error: `Offscreen not ready: ${e?.message || e}`,
        });
        return;
      }
      ...
      try {
        ...
        if (r?.ok) {
          ...
          setBadge(true);
          chrome.runtime
            .sendMessage({ type: "RECORDING_STATE", recording: true })
            .catch(() => {});
        ...
        sendResponse({
          ok: false,
          error: `OFFSCREEN_START failed: ${e?.message || e}`,
        });
      }
      return;
    }
    ...
    if (msg?.type === "GET_TRANSCRIPT_TEXT") {
